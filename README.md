...
## q1-25divDATES.sh
Dividend Finance Scheduling data report.  Note the asterisks on the Declaration Date to indicate stocks with near future declarations.  When the Declaration Date matches the day the script is run, it will display a leading asterisk(*).  Also, if the Pay Date is within five days, it will add a leading asterisk.  In the example below, the report dates are highlighted as if it was run on 01/03/2025
Example output:
```term
> sh q1-25divDATES.sh
